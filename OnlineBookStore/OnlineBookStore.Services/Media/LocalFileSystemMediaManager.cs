...
namespace OnlineBookStore.Services.Media
{
    public class LocalFileSystemMediaManager : IMediaManager
    {
        private const string PicturesFolder = "uploads/{0}/{1}{2}";
        ...
        public Task<bool> DeleteFileAsync(string filePath, CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                var fullPath = Path.GetFullPath(Path.Combine(PicturesFolder, filePath));
        ...
        public async Task<string> SaveFileAsync(Stream buffer, string originalFileName, string contentType, CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                var fullPath = @"C:/xampp/htdocs/online-book-store/" + Path.Combine(returnedFilePath);
        ...
        {
            return string.Format(PicturesFolder, contentType.Equals("application/pdf") ? "files" : "cover", Guid.NewGuid().ToString("N"), fileExt);
        }
    }
}
